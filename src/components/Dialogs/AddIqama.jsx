...
import { addiqamatimingsapi } from '../API/Api';

export function AddIqama({ open, setOpen }) {
  ...
  const { mutate, isLoading } = useMutation({
    mutationFn: (data) => addiqamatimingsapi(data.date, data.filteredTimes),
    onSuccess: () => {
      setDate(null);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent
        className="sm:max-w-[825px] mt-8 top-[15%] transform transition-transform duration-500 ease-out -translate-y-20"
      >
        <DialogHeader>
          <DialogTitle>Add Iqamah</DialogTitle>
        </DialogHeader>
        <div>
          <div>
